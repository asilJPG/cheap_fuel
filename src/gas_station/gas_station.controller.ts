...
export class GasStationController {
  ...
  async createGasStation(
    ...
    const gasStation = await this.gasStationService.createGasStation(
      createGasStationDto,
    );
    return gasStation;
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Updated gas station',
    type: [GasStation],
  })
  @Put(':id')
  async updateGasStationById(
    @Param('id') id: string,
    @Body() updateComanyDto: UpdateGasStationDto,
  ): Promise<GasStation> {
    const gasStation = await this.gasStationService.updateGasStationById(
      +id,
      updateComanyDto,
    );
    return gasStation;
  ...
  @ApiResponse({
    status: 200,
    description: 'Deleted gas station',
    type: [GasStation],
  })
  @Delete(':id')
