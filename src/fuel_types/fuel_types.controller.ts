...
export class FuelTypeController {
  ...
  @Put(':id')
  async updateFuelTypeById(
    @Param('id') id: string,
    @Body() updateComanyDto: UpdateFuelTypeDto,
  ): Promise<FuelType> {
    const fuelType = await this.fuelTypeService.updateFuelTypeById(
      +id,
      updateComanyDto,
    );
    return fuelType;
